...
A component is one typed content unit such as `bullets`, `chart`, `blockquote`, `image`, or `sankey`. Components fill pages after the spec has chosen their semantic move. The current IR exposes 62 component types.
