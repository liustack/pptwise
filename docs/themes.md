...
The shared `LAYOUT_REGISTRY` contains 117 entries: 113 standard layouts and 4 image takeovers. Of the standard layouts, 43 belong to the shared auto-selectable pool and 70 are `pinOnly`. The theme redesign waves raised the pin-only count by adding dedicated cover, chapter, and ending constructions for the built-in themes. These constructions stay out of the shared automatic pool. At the authoring surface they are reached through explicit `slide.layout` pins, including layout ids materialized from board locks.

Cover, chapter, and ending pages now lock to their Claude Design board construction. Soft preference cannot hold those pages in place. Content pages still pick from the auto-selectable set (9 ids after `banner-heading` retired), weighted by the allocation table. lecture and luxe drop `split-band` / `stacked-poster`. Playbill keeps the full auto content set.
