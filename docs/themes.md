...
The shared `LAYOUT_REGISTRY` contains 122 entries: 118 standard layouts and 4 image takeovers. Of the standard layouts, 47 belong to the shared auto-selectable pool and 71 are `pinOnly`. Theme redesign waves supplied most pin-only constructions. The consulting gauge family adds four automatic layouts and the pin-only `gauge-point`. Pin-only layouts stay out of the shared automatic pool and are reached through explicit `slide.layout` pins, including layout ids materialized from board locks.

Cover, chapter, and ending pages now lock to their Claude Design board construction. Soft preference cannot hold those pages in place. The registry exposes 10 auto-selectable content layouts, while existing built-ins preserve the prior 9-id shared set so the consulting addition cannot reshuffle their seeded picks. lecture and luxe drop `split-band` and `stacked-poster` from that baseline. consulting locks content to `gauge-stats`. A registered custom theme that omits `layouts` receives all 10 ids.
