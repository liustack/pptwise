...
describe("RailNumberedContent", () => {
  ...
  it("paints no left rail track (author ruling 2026-09-01), keeps the number badge clear of all four logo bands, no foreignObject", () => {
    ...
    // The former progress track — rect(48, 96, 4, 544) — was cut: across
    // themes it read as a stray vertical line beside the content. Pin its
    // absence so it cannot quietly come back.
    const rail = Array.from(root.querySelectorAll("rect")).find(
      (r) => r.getAttribute("x") === "48" && r.getAttribute("width") === "4",
    )
