 ...
 * 一条 hairline 分隔的"联系"区块 + 版权行。自 templates/thesis.tsx 的
 * `BCGEmeraldEnding` 提炼。
 *
 ...
 * thesis.tsx 里只有本函数消费），同 chapter-rail-chapter.tsx 处理
 ...
 * thesis 自己的实测：`0x8A968F` 对 thesis 真实渲染背景 `#FAFAF6` 只有
 ...
 *   版权行 fill 用的一个内联十六进制字面量，在 thesis.ts 的 colors 表里
 *   没有精确匹配——整个 thesis.tsx 文件 grep 只出现这一处（不是模块级
 *   具名常量）。判断过程（Step B）：
 ...
 *      `TRIANGLE_DEEP` 一类。
 *   3. 跨主题旁证——`templates/brief.tsx` 的 `MckinseyNavyEnding`
 *      版权行也独立烤了一个跟自己 `muted` 不同的近似灰（brief 自己的
...
// 随迁自 thesis.tsx 模块作用域（555-557 行），只有 BCGEmeraldEnding 消费，
