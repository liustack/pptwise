 ...
 * accent 色下划线。自 templates/brief.tsx 的 `MckinseyNavyChapter`（184-265
 * 行，非计划原文估计的 184-324——已按 Step A 用 `awk` 精确定位函数起止）
 * 提炼。无随迁 helper。
 *
 * 替换表（沿用 P1 已验证的 brief 替换表，先例 cover-banner-title.tsx，
 * 十六进制值本身不抄进本注释，避免污染本文件的 grep 清零门，同
 * chapter-rail-chapter.tsx 先例）：
 *   YELLOW → ctx.colors.accent（逐字符核对 themes/brief.ts 的
 ...
 * 默认背景总是深色，全集放开后对 bulletin/heritage/ink/journal/
 ...
 * chapter.tsx 同一根因）：副题固定 0.7 透明度，homeroom 的章节默认背景
