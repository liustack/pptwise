 ...
 * 的 `section#g5` brief 设计表，几何坐标逐条抄录，不派生）。
 *
 ...
 *
 * ## 第四轮评审砍掉的两件（2026-08-20，thesis p01/p09）
 *
 * 本 motif 的锚点主题是 brief，bulletin 借用（thesis 已在
 * board-cover-restore wave 2 钉成 `rail-motif` 单成员）。用户在
 * thesis 上逐条点名（thesis 的 primary 是祖母绿，所以原话说的是绿线）：
 ...
 * 这一轮都没被点名。留它也是为了 brief（本 motif 的锚点主题，本轮
 * 零条返工）不会只剩一张白纸。
 *
 * chapter 完全退让（`return null`）：brief 的 chapter 默认底色就是
 * 整版 `primary` 藏青（`themes/brief.ts` 的 `defaultBackgrounds
 * .chapter`），两条线走的也是 `primary`，同色压同色实测 **1.00:1**；借用
 * 本 motif 的 thesis 同构（chapter 底色 = 自己的 primary，同样 1.00:1）。
 * 三件里两件在两家上直接消失，剩一枚孤零零的黄块不成语汇，所以整档退让
 * ——与 clinic/bulletin/rail/homeroom 本轮统一的 chapter 处理一致。
 * bulletin 借用时 chapter 底色是浅灰（primary 压它 9.96:1，本来画得出），
 ...
 * （柔和组建，本轮复用并自校验：homeroom 那次的 486 条文字 / 顶沿 y34 /
 * 左沿 x56 / 右沿 x1224 逐个复现）。把 `LAYOUT_REGISTRY` 全部版式 + 主题
 * 十页 deck 在 brief / thesis / bulletin 三家上各渲一遍，非 chapter
 ...
 *      （thesis 的点轨、homeroom 的铅笔虚线，都是板上写 y640/y648
 *      实测后搬走）。
...
export function BannerMotif({ slide, ctx }: DecorProps) {
  // chapter 是整版 primary 底（brief/thesis 同构），线同色压同色
  // 1.00:1——见文件头。
