<p align="center"><img src="assets/banner.png" alt="pptpress — make your deck in minutes, not hours" width="100%"></p>

<h1 align="center">pptpress</h1>
...
<p align="center">
  ...
  <a href="./skills/pptpress/SKILL.md">Agent skill</a> ·
...
<p align="center">
  ...
  <a href="https://www.npmjs.com/package/@liustack/pptpress"><img src="https://img.shields.io/npm/v/@liustack/pptpress?style=flat-square&label=npm&color=cb3837" alt="npm"></a>
  <a href="https://nodejs.org"><img src="https://img.shields.io/node/v/@liustack/pptpress?style=flat-square" alt="Node.js"></a>
...
Issues are welcome any time. [Open one](https://github.com/liustack/pptpress/issues/new/choose), or follow **[@liustack](https://x.com/liustack)** on X. Share what you made with pptpress, which harness you use, and what the next release should solve. New releases land there first.
...
**🔌 Installs into the agent you already use.** One command puts pptpress into DeepSeek Harness, Claude Code, or any agent that reads a skill folder (Codex and friends), and it knows how to build a deck the moment it lands.
...
EOF
pptpress validate deck.json                              # → OK — 3 slides, theme "consulting"
pptpress render deck.json -o out/hello.pptx              # → wrote out/hello.pptx (3 slides, ~24 KB)
pptpress render deck.json -o out/tech.pptx --theme tech  # same deck, different theme
pptpress preview deck.json -o out/svgs                   # SVG per slide, for a visual self-check
```
...
No install at all also works: `npx -y @liustack/pptpress validate deck.json`. In a source checkout, `node dist/cli.js` replaces `pptpress`, and `examples/` has ready-made IR files to try.
...
| `render <target> [-o <out.pptx>] [--theme <id>]` | Render a `.pptx`. Omit `-o` to write `.pptpress/<deck>/<deck>.pptx` |
| `preview <target> [-o <dir>] [--html]` | One SVG per slide, plus a self-contained review page. Omit `-o` to write `.pptpress/<deck>/` |
...
| [Agent skill](./skills/pptpress/SKILL.md) | Learning the workflow pptpress teaches an agent |
...
Icon primitives are extracted from [lucide](https://lucide.dev) (ISC License). pptpress itself was extracted from a production AI-deck-generation system and CJK-typography-tuned (full-width punctuation width, Chinese line breaking, a Chinese-first font stack, explicit east-asian font-slot declarations) from day one.
